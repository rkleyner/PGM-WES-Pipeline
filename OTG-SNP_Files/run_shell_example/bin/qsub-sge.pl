...
=head1 Usage
  ...
  --priority <str>  the priority of the department in hongkong, such as bc_phar
...
GetOptions(
	...
	"interval:i"=>\$Interval,
	"queue:s"=>\$Queue,
	"priority:s"=>\$Priority,  #added by huangkai
	"convert:s"=>\$Convert,
	"secure:s"=>\$Secure,
	"reqsub"=>\$Reqsub,
	"resource:s"=>\$Resource,
...
while (@Shell) {
	...
	$job_cmd .= "-P $Priority " if (defined $Priority); # added by huangkai 20111116 set priority
